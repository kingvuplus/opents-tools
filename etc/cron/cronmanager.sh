...
#
echo "-----------------------------------------------------"
echo "   *** Cronmanager Version $VERSION by gutemine ****  "
echo "-----------------------------------------------------"
#
...
case "$1" in
   ...
   echo "-----------------------------------------------------"
   if [ -f $CMHOMETABS/root ]; then
      $CRONTAB -c /etc/cron/crontabs -l
   else
      echo "no cronjobs found; use 'add command' to create conjob(s)"
   fi
   echo "-----------------------------------------------------"
   ;;
   "delete")
   echo "Deleting crontab ..."
   $CRONTAB -c /etc/cron/crontabs -r
   echo "-----------------------------------------------------"
   ;;
   "info")
   echo "Check next line if cron dameon is running"
   echo "-----------------------------------------------------"
   ps ax | grep /usr/sbin/crond | grep crontab
   echo "-----------------------------------------------------"
;;
   "enable")
   ...
   ln -sfn $CRONDAEMONSCRIPT /etc/rc3.d/S99cron
...
cat $CRONTABFILE
$CRONTAB -c /etc/cron/crontabs -l > $CRONTABFILE 
echo $FULLCOMMAND >> $CRONTABFILE
sed -ie s!"%"!!g $CRONTABFILE
#echo "--------------------------------------------------"
#echo "appending line to crontab now ..."
$CRONTAB -c /etc/cron/crontabs $CRONTABFILE
echo "--------------------------------------------------"
...
;;
  *)
    ...
    echo "                       enable|disable|reboot|halt|kill}"
    echo "-----------------------------------------------------"
